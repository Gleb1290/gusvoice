...
fn main() {
    ...
    match target_os.as_str() {
        "windows" => {
            ...
            // GusVoice NVENC fork: hardware H.264/H.265 encode on Windows.
            // Mirrors the Linux nvidia block but for MSVC. The CUDA driver
            // headers + import lib come from the cuda_cudart redist archive
            // pointed to by GV_CUDA_DIR (no full CUDA Toolkit / admin install
            // needed). NVENC itself (nvEncodeAPI64.dll) is loaded dynamically
            // by NvEncoder.cpp, so only cuda.lib (the driver API) is linked.
            // ENCODER-ONLY: the nvidia decoder (NVDEC/nvcuvid) is excluded
            // (`!_WIN32` guard in video_decoder_factory.cpp) — viewers decode
            // in their own browser. Absent GV_CUDA_DIR we fall back to the
            // software OpenH264 path, exactly like upstream.
            if let Ok(cuda_dir) = env::var("GV_CUDA_DIR") {
                let cuda_dir = PathBuf::from(cuda_dir);
                ...
                if cuda_include.join("cuda.h").exists() {
                    ...
                    println!(
                        "cargo:warning=GusVoice: building WITH NVENC hardware H.264/H.265 (CUDA at {})",
                        cuda_dir.display()
                    );
                } else {
                    println!(
                        "cargo:warning=GV_CUDA_DIR set but cuda.h not found at {}; NVENC disabled",
                        cuda_include.display()
                    );
                }
            } else {
                println!("cargo:warning=GV_CUDA_DIR not set; building WITHOUT NVENC (software H.264)");
            }
        }
        "linux" => {
