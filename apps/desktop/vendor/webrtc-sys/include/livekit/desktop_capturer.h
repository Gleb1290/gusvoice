...
// GusVoice: включить/выключить WGC для захвата ОКНА (см. комментарий в .cpp — утечка в explorer).
// Глобально, а не полем опций: структуру опций конструирует обёртка livekit-webrtc, которую мы не
// вендорим, а трогать её ради одного флага пришлось бы по всей цепочке крейтов.
