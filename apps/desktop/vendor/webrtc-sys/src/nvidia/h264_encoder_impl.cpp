...
NvidiaH264EncoderImpl::NvidiaH264EncoderImpl(
  ...
  // GusVoice fix: upstream never initialised nv_profile_guid_, so InitializeEncoder
  // got a garbage profile GUID -> NV_ENC_ERR_INVALID_PARAM (error 8) and the encoder
  // silently fell back to software OpenH264. Map the negotiated H264 profile to the
  // matching NVENC profile GUID (the H265 impl already does the equivalent).
  switch (profile_) {
    ...
    case H264Profile::kProfilePredictiveHigh444:
      nv_profile_guid_ = NV_ENC_H264_PROFILE_HIGH_444_GUID;
      break;
  ...
  // GusVoice fix: the negotiated SDP level (e.g. 42e01f = baseline level 3.1) caps
  // resolution well below what we stream (1080p/1440p). Forcing that level made NVENC
  // reject InitializeEncoder (INVALID_PARAM). Let NVENC auto-pick a level that fits
  // the actual encode resolution.
...
void NvidiaH264EncoderImpl::SetRates(
  ...
  // GusVoice fix: actually PUSH the new bitrate/framerate into NVENC. Upstream only
  // updated these local fields and never called Reconfigure, so the encoder stayed
  // locked at the initial (low) startBitrate — WebRTC's bandwidth estimate ramps the
  // target up to several Mbps but the encoder kept CBR-quantising at a few hundred
  // kbps, which at 1080p looks like heavy blocking that "falls apart" even on static
  // content. Reconfigure the rate control live (no encoder reset, no forced IDR).
  if (configuration_.target_bps) {
    const uint32_t fps =
        std::max<uint32_t>(1, static_cast<uint32_t>(parameters.framerate_fps));
    const uint32_t bps = static_cast<uint32_t>(configuration_.target_bps);
    nv_initialize_params_.frameRateNum = fps;
    nv_initialize_params_.frameRateDen = 1;
    nv_encode_config_.rcParams.averageBitRate = bps;
    nv_encode_config_.rcParams.maxBitRate = bps;  // CBR: cap == average
    // ~1 frame of VBV keeps screen-share latency low while still hitting the target.
    nv_encode_config_.rcParams.vbvBufferSize = bps / fps;
    nv_encode_config_.rcParams.vbvInitialDelay =
        nv_encode_config_.rcParams.vbvBufferSize;

    NV_ENC_RECONFIGURE_PARAMS reconfig = {};
    reconfig.version = NV_ENC_RECONFIGURE_PARAMS_VER;
    reconfig.reInitEncodeParams = nv_initialize_params_;  // .encodeConfig -> nv_encode_config_
    reconfig.resetEncoder = 0;
    reconfig.forceIDR = 0;
    try {
      if (encoder_->Reconfigure(&reconfig)) {
        RTC_LOG(LS_VERBOSE) << "NVENC reconfigured: " << bps << " bps @ " << fps
                            << " fps";
      } else {
        RTC_LOG(LS_WARNING) << "NVENC Reconfigure returned false (bps=" << bps
                            << ")";
      }
    } catch (const NVENCException& e) {
