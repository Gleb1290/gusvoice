...
void NvidiaH265EncoderImpl::SetRates(
  ...
  // GusVoice fix (ported from the H.264 path): actually PUSH the new bitrate/framerate
  // into NVENC. Upstream only updated these local fields and never called Reconfigure, so
  // the encoder stayed locked at the initial (low) startBitrate — WebRTC's bandwidth
  // estimate ramps the target up to several Mbps but the encoder kept CBR-quantising at a
  // few hundred kbps, which at 1080p looks like heavy blocking that "falls apart" even on
  // static content. Reconfigure the rate control live (no encoder reset, no forced IDR).
  if (configuration_.target_bps) {
    ...
    try {
      if (encoder_->Reconfigure(&reconfig)) {
        RTC_LOG(LS_VERBOSE) << "NVENC(H265) reconfigured: " << bps << " bps @ "
                            << fps << " fps";
      } else {
        RTC_LOG(LS_WARNING) << "NVENC(H265) Reconfigure returned false (bps="
                            << bps << ")";
      }
    } catch (const NVENCException& e) {
