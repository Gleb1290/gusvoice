...
// GusVoice: переключатель захвата ОКНА через Windows Graphics Capture.
//
// WGC подтекает в процесс проводника: у одного человека explorer.exe рос со 150 до 750 МБ за два
// часа показа окна, три сессии подряд, а с концом показа рост прекращался. Когда проводнику
// становится плохо, отваливается ровно то, что он обслуживает, — Alt+Tab, «Пуск», панель задач;
// Alt+F4 и диспетчер задач при этом работают, потому что идут мимо него. Тот же баг и та же причина
// известны у TeamSpeak 6, там его закрыли отключением WGC.
//
// Захват ЭКРАНА не трогаем: он идёт через DirectX и у показывающих экран утечки нет вовсе (семь
// сессий разных людей). Флаг только для окна — и по умолчанию WGC ОСТАЁТСЯ включённым: без него
// libwebrtc падает на старый метод, который плохо снимает окна игр с аппаратным рендерингом.
static std::atomic<bool> g_allow_wgc_window{true};

void set_wgc_window_capture(bool enabled) {
  g_allow_wgc_window.store(enabled, std::memory_order_relaxed);
}
...
std::unique_ptr<DesktopCapturer> new_desktop_capturer(
  ...
  switch (options.source_type) {
    ...
    case SourceType::Window:
      webrtc_options.set_allow_wgc_window_capturer(
          g_allow_wgc_window.load(std::memory_order_relaxed));
