...
// GusVoice: NVENC fork compiles the encoder ONLY on Windows (viewers decode in
// their own browser via NVDEC), so the nvidia DECODER factory is excluded there.
#if defined(USE_NVIDIA_VIDEO_CODEC) && !defined(_WIN32)
